...
const ReserveContents = ({
  ...
  const { isOpenForm, toggleForm } = useOpenForm();
  ...
  return (
    <>
        ...
        <InfoContainer>
          ...
          <InfoDetailContainer>
            ...
            {showButton && status === 'ok' ? (
              <Button
                className='absolute right-8 top-48 h-[44px] w-[146px] rounded-[10px] font-bold'
                onClick={() => toggleForm()}
              >
                리뷰 작성
              </Button>
      ...
      {isOpenForm && <ReviewForm reservation={reservation} />}
    </>
  );
};
